...
export function FinalCta({ isLoggedIn = false }: { isLoggedIn?: boolean }) {
    ...
    <section ref={ref} className="relative overflow-hidden bg-gradient-to-br from-primary via-primary to-secondary py-16 sm:py-20">
      ...
      <div className="cta-inner relative mx-auto max-w-3xl px-4 text-center">
        <h2 className="font-heading text-3xl font-bold tracking-tight text-white sm:text-4xl">
          {isLoggedIn ? "Lanjutkan mapping-mu" : "Siap petakan ide besarmu?"}
        </h2>
        <p className="mx-auto mt-3 max-w-xl text-white/80">
          {isLoggedIn ? "Kembali ke dashboard dan lanjutkan kolaborasi dengan tim." : "Bergabung dengan tim yang sudah memetakan ribuan ide di Mapping. Gratis, tanpa kartu kredit."}
        </p>
        <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
          <Link
            href={isLoggedIn ? "/maps" : "/register"}
            className="flex items-center gap-2 rounded-lg border-2 border-foreground bg-card px-8 py-3.5 text-base font-bold text-foreground shadow-brutal transition-all hover:translate-x-[-1px] hover:translate-y-[-1px] hover:shadow-brutal-lg"
          >
